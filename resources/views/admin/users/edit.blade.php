...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h4 class="mb-0 text-dark fw-bold">
        <i class="fa-solid fa-user-pen text-warning"></i> Edit Data Pengguna
    </h4>
...
<div class="table-card-wrapper shadow-sm bg-white p-4 rounded">
    ...
    <form action="{{ route('admin.users.update', $user->id) }}" method="POST" autocomplete="off">
        ...
        {{-- Baris 3: Password baru (opsional) + Konfirmasi --}}
        <div class="row mb-3">
            <div class="col-md-6">
                <label class="form-label fw-bold">
                    Password Baru
                    <small class="fw-normal text-muted">(kosongkan jika tidak diubah)</small>
                </label>
                <div class="input-group">
                    ...
                    <button class="btn btn-outline-secondary" type="button"
                            onclick="togglePassword('password', this)" title="Tampilkan/sembunyikan">
            ...
            <div class="col-md-6">
                ...
                <div class="input-group">
                    <input type="password"
                           ...
                           autocomplete="new-password">
                    <button class="btn btn-outline-secondary" type="button"
                            onclick="togglePassword('password_confirmation', this)" title="Tampilkan/sembunyikan">
                        <i class="fa-solid fa-eye"></i>
                    </button>
                </div>
        ...
        {{-- Baris 4: Tempat/Tgl Lahir + Jenis Kelamin --}}
        <div class="row mb-3">
            <div class="col-md-4">
        ...
        <div class="row mb-3">
            <div class="col-md-4">
                ...
                <select name="wilayah_id" class="form-select @error('wilayah_id') is-invalid @enderror">
                    ...
                    @foreach($wilayahs as $wil)
                        <option value="{{ $wil->id }}"
                            {{ old('wilayah_id', $user->wilayah_id) == $wil->id ? 'selected' : '' }}>
                            {{ $wil->nama_wilayah }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-md-4">
                <label class="form-label fw-bold">Tekanan Darah <small class="fw-normal text-muted">(Opsional, cth: 120/80)</small></label>
                <input type="text"
                       name="tekanan_darah"
                       class="form-control @error('tekanan_darah') is-invalid @enderror"
                       value="{{ old('tekanan_darah', $user->tekanan_darah) }}"
                       placeholder="120/80">
                @error('tekanan_darah')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
        ...
        <div class="text-end">
            <button type="submit" class="btn btn-warning">
                <i class="fa-solid fa-save me-1"></i> Perbarui Data Pengguna
            </button>
        </div>
    </form>
</div>
...
// Real-time password match indicator (only active when password field has value)
var pwInput   = document.getElementById('password');
var pwConfirm = document.getElementById('password_confirmation');
var hint      = document.getElementById('pw-match-hint');

function checkMatch() {
    if (!pwInput.value)    { hint.textContent = ''; hint.className = 'form-text'; return; }
